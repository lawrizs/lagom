...
public enum CosmosRetryPolicyOption implements CosmosDriverOption {
   ...
   *
   * <p>Delays are triggered in response to {@link OverloadedException} errors. The delay time is
   * extracted from the error message, or--if the delay time isn't present in the error
   * message--computed as follows:
   *
   * <p>
   *
   * <pre>{@code
  ...
  /**
   * Growing back-off time for delayed retries triggered by {@link
   * CosmosRetryPolicy#onErrorResponse}.
   *
   * <p>Delays are triggered in response to {@link OverloadedException} errors. The delay time is
   * extracted from the error message, or--if the delay time isn't present in the error
   * message--computed as follows:
   ...
   * }</pre>
   */
  GROWING_BACKOFF_TIME(
      "growing-backoff-time",
      (option, profile) -> profile.getInt(option, option.getDefaultValue(Integer.class)),
      Integer::parseUnsignedInt,
      1_000),
   ...
   *
   * <p>Disabling read timeout retries may be desirable when Cosmos DB server-side retries are
   * enabled.
   */
   ...
   *
   * <p>Disabling write timeout retries may be desirable when Cosmos DB server-side retries are
   * enabled.
   */
