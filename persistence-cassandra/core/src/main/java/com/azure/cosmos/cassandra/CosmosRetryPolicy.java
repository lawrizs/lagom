 ...
 *
 * <p>This is the default retry policy when you take a dependency on {@code
 * azure-cosmos-cassandra-driver-4-extensions}. It provides a good out-of-box experience for
 * communicating with Cosmos Cassandra API instances. Its behavior is specified in configuration:
 *
 * <p>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The number of retries that should be attempted is specified by {@code max-retries}. A value of
 * {@code -1} indicates that an indefinite number of retries should be attempted.
 *
 * <p>Retry decisions by {@link #onErrorResponse} may be delayed. Delays are triggered in response
 * to {@link OverloadedException} errors. The delay time is extracted from the error message,
 * or&mdash;if the delay time isn't present in the error message&mdash;computed as follows:
 *
 * <p>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>There are no other uses for {@code fixed-backoff-time} and {@code growing-backoff-time}. Retry
 * decisions are returned immediately in all other cases. There is no delay in returning from any
 * method but one: {@link #onErrorResponse} when it receives an {@link OverloadedException} error.
 *
 * @see <a href="../../../../../doc-files/reference.conf.html">reference.conf</a>
 * @see <a
 *     href="https://docs.datastax.com/en/developer/java-driver/4.11/manual/core/retries/">DataStax
 *     Java Driver Retries</a>
 */
public final class CosmosRetryPolicy implements RetryPolicy {
  ...
  public CosmosRetryPolicy(final DriverContext driverContext, final String profileName) {
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          "CosmosRetryPolicy(sessionName: {}, profileName: {})",
   ...
   *
   * <p>A value of {@code -1} indicates that an indefinite number of retries should be attempted.
   *
   ...
   *
   * <p>This value indicates whether read timeouts are enabled.
   *
   * @return The {@code retry-read-timeouts} value specified by this {@link CosmosRetryPolicy}
   *     object.
   */
   ...
   *
   * <p>This value indicates whether write timeouts are enabled.
   *
   * @return The {@code retry-write-timeouts} value specified by this {@link CosmosRetryPolicy}
   *     object.
   */
  ...
  /**
   * Returns a retry decision in response to a recoverable error other than READ_TIMEOUT,
   * WRITE_TIMEOUT or UNAVAILABLE.
   *
   * <p>Recoverable errors are represented by these exception types:
   *
   * <p>
   *
   * <ul>
   *   <li>{@link OverloadedException},
   *   <li>{@link ServerError},
   *   <li>{@link TruncateException},
   *   <li>{@link ReadFailureException}, and
   *   <li>{@link WriteFailureException}.
   * </ul>
   *
   * <p>The following errors are handled internally by the driver, and therefore will never be
   * encountered by this method:
   *
   * <p>
   *
   * <ul>
   *   <li>{@link BootstrappingException} is always retried on the next node;
   *   <li>{@link QueryValidationException} (and its subclasses),
   *   <li>{@link FunctionFailureException}, and
   *   <li>{@link ProtocolError}: always rethrown.
   * </ul>
   *
   * <b>Note:</b> This method will only be invoked for {@link Request#isIdempotent idempotent}
   ...
   *     <ul>
   *       <li>{@link RetryDecision#RETRY_SAME Retry on the same node}, if an {@link
   *           OverloadedException} or {@link WriteFailureException} is received and the {@code
   *           retryCount} is less than {@link #getMaxRetryCount};
   *       <li>{@link RetryDecision#RETHROW} otherwise.
   *     </ul>
   *     <p>When retrying, this method pauses for the duration of the retry interval extracted from
   *     {@link CoordinatorException#getMessage error message} or--if there is no retry interval in
   *     the error message--as computed from the {@link CosmosRetryPolicyOption options} specified
   *     for this {@link RetryPolicy}. If {@link #getMaxRetryCount}
   *     <p>
   *     <pre>{@code
   ...
   * }</pre>
   *     <p>Or, if {@link #getMaxRetryCount} is {@code -1}:
   *     <p>
   *     <pre>{@code
  ...
  public RetryDecision onErrorResponse(
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          "onErrorResponse(request: {}, error: {}, retryCount: {})",
          toJson(request),
          toJson(error),
          toJson(retryCount));
    }
        ...
        if (retryAfterMillis == -1) {
          retryAfterMillis =
              this.maxRetryCount == -1
                  ...
                  : retryCount * this.growingBackOffTimeInMillis
                      + RANDOM.nextInt(GROWING_BACKOFF_SALT_IN_MILLIS);
        }
        if (LOG.isTraceEnabled()) {
          LOG.trace(
  ...
  /**
   * Returns a {@link RetryDecision#RETRY_SAME retry-same} or {@link RetryDecision#RETHROW} decision
   * in response to a {@link ReadTimeoutException read timeout error}.
   *
   * <p>Retries on read timeouts can be disabled by setting {@link
   * CosmosRetryPolicyOption#READ_TIMEOUT_RETRIES} to {@code false}:
   *
   * <p>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * Disabling retries for read timeouts may be desirable when Cosmos DB server-side retries are
   * enabled.
   *
   * @param request The request that failed.
   ...
   * @param received The number of replica acknowledgements/responses to the operation before it
   *     failed.
   ...
   * @param retryCount The current retry count.
   * @return A maximum of one {@link RetryDecision#RETRY_SAME retry to the same node}, if the number
   *     of replica acknowledgements/responses received before the operation failed (the value of
   *     {@code received}) is greater than or equal to the minimum number of replica
   *     acknowledgements/responses required to fulfill the operation (the value of {@code
   *     blockFor}), data was not retrieved ({@code dataPresent}: false}, and only if retries on
   *     read timeouts are enabled (the default). Otherwise, a {@link RetryDecision#RETHROW rethrow
  ...
  /**
   * Returns a {@link RetryDecision#RETRY_SAME retry-same} or {@link RetryDecision#RETHROW} decision
   * when a request is aborted before a response is received.
   *
   * <p>This can happen in two cases:
   *
   * <p>
   *
   * <ul>
   *   <li>The connection was closed due to an external event. This will manifest as a {@link
   *       ClosedConnectionException} (network failure) or a {@link HeartbeatException} (missed
   *       heartbeat); or
   *   <li>There was an unexpected error while decoding the response. This can only be a driver bug.
   * </ul>
   *
   * <p>This method is only invoked for idempotent statements.
   *
   * @param request The request that failed.
   * @param error The error received.
   * @param retryCount The current retry count.
   * @return A {@link RetryDecision#RETRY_SAME retry-same decision}, if a {@link
   *     ClosedConnectionException} or {@link HeartbeatException} is received and {@code retryCount}
   *     is less than {@link #getMaxRetryCount}; otherwise, a {@link RetryDecision#RETHROW rethrow}
   *     decision.
   */
  @Override
  public RetryDecision onRequestAborted(
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          "onRequestAborted(request: {}, error: {}, retryCount: {})",
          toJson(request),
          toJson(error),
          toJson(retryCount));
    }
   ...
   * @param retryCount The current retry count.
   * @return A {@link RetryDecision#RETRY_SAME retry-same decision}, if {@code retryCount} is less
   *     than the {@link #getMaxRetryCount}.
   */
  @Override
  ...
  /**
   * Returns a {@link RetryDecision#RETRY_SAME retry-same} or {@link RetryDecision#RETHROW rethrow}
   * decision in response to a {@link WriteTimeoutException write timeout error}.
   *
   * <p>This method is only invoked for idempotent statements. It is similar to {link
   * #onReadTimeout}, but for write operations.
   *
   * <p>Retries on write timeouts can be disabled by setting {@link
   * CosmosRetryPolicyOption#WRITE_TIMEOUT_RETRIES} to {@code false}:
   *
   * <p>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * Disabling retries for write timeouts may be desirable when Cosmos DB server-side retries are
   * enabled.
   *
   * @param request The request that failed.
   * @param consistencyLevel The consistency level of the request that failed.
   ...
   *     {@link WriteType#BATCH_LOG batch log} operations, if and only if retries on write-timeouts
   *     are enabled (the default). Otherwise, a {@link RetryDecision#RETHROW rethrow decision} is
   *     returned.
   */
  @Override
   ...
   *
   * <p>Here is an example error message:
   *
   * <p>
   *
   * <pre>{@code
