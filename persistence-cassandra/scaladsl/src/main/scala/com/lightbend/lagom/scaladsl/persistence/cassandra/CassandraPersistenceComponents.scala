...
import com.lightbend.lagom.internal.persistence.cassandra.{CassandraOffsetStore, CassandraReadSideSettings, ServiceLocatorHolder}
...
trait WriteSideCassandraPersistenceComponents extends WriteSidePersistenceComponents {
  ...
  private[lagom] val serviceLocatorHolder: ServiceLocatorHolder = {
    ...
    holder.setServiceLocator((name: String) => serviceLocator.locateAll(name))
    holder
  }
}
